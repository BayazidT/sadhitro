...
<?php
  ...
  <div class="content-wrapper">
    <!-- Content Header (Page header) -->
      ...
      <div>
                  <ol class="breadcrumb">
                      <li><a href="#">TUTOR/<?php echo $search; ?> </a></li>
                </ol>
                     
              </div>
              <?php

            $cQuery = "SELECT * FROM s_tutor  WHERE (name LIKE '%$search%')  OR (subject LIKE '%$search%') OR (standard LIKE '%$search%')  OR (description LIKE '%$search%') OR (location LIKE '%$search%') limit $start_form, $per_page";
            $qResult = $db->select($cQuery);
            if($qResult){
              while($result = $qResult->fetch_assoc()){
                ?>
